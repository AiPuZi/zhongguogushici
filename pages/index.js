  ...
  {Array.isArray(poetryData) && poetryData.map((poem, index) => (
    <div key={index} className="poem">
      ...
      <div className="attribution">
        ...
        <br />第一次打开网站首页点击其他分类时，会自动跳回首页。第二次之后点击就不会。
        <br />第一次点击下一页按钮时需要点击两次才有反应，以后就不用。唐诗分类第一次可能要点击几次。
        <br />全唐诗分类诗词数量非常多，所以从其他分类点击回这个分类时加载较缓慢，要等几秒才能显示。
        <br />搜索的时候比较缓慢，需要等8秒左右才能显示。如果搜索的内容不存在，页面会显示空白。
